...
const Location: NextPage = () => {
	...
	const breeds = useQuery<Dog[], any, Dog[], any>({
		queryKey: ["breeds"],
		queryFn: () => DogService.getBreeds(),
		select: (data: Dog[]) => data.filter((breed) => breed.img),
	});
	const {
		data,
	...
	return (
		<>
			<Container className="!static">
				<ContainerHeader className="flex-wrap" title="location">
					<Button
				...
				<form className={classes.location__options} onSubmit={onSubmit}>
					<div>
						<div className={classes.location__options__title}>
							order
						</div>
						<Select
							className={classes.location__options__select}
							value={options.order}
							onChange={(e) =>
								setOptions({
									...options,
